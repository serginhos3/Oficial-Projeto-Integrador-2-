<x-app-layout>
            ...
            <div class="bg-white shadow-md rounded-lg overflow-hidden">
                <div class="p-6">
                    <div class="flex justify-between mb-4 items-start">
                        <input type="text" id="myInputTextField" placeholder="Pesquisar..."
                            class="border border-gray-300 rounded-md p-2 mb-4">

                        <a href="{{ route('pacientes.cadastrar') }}"
                            class="bg-gray-800 text-white py-2 px-4 rounded-md hover:bg-gray-700 transition duration-300">CADASTRAR</a>
                        ...
                        <table id="pacientesTable" class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-200">
                                <tr>
                                    ...
                                    <th
                                        class="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider w-24">
                                        Ações</th>
                                </tr>
                            </thead>
                                ...
                                @foreach ($pacientes as $paciente)
                                    <tr class="hover:bg-gray-100 transition duration-150">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            {{ $paciente->datanascimento->format('d/m/Y') }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex justify-center gap-2">
                                                <a href="{{ route('pacientes.editar', $paciente->id) }}"
                                                    class="text-black border-0 bg-transparent hover:bg-transparent"
                                                    title="Editar">
                                                ...
                                                <form action="{{ route('pacientes.destroy', $paciente->id) }}"
                                                    ...
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="text-red-600" title="Excluir"
                                                        onclick="return confirm('Você tem certeza que deseja excluir este paciente?');">
    ...
    <script>
        $(document).ready(function() {
            let oTable = $('#pacientesTable').DataTable({
                searching: true,
                language: {
